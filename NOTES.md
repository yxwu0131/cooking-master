...
- **第二批硬化（同日，tsc + next build 全过）**：把评测里「该做的低优先项」补完——① `callDeepSeek` 加轻量重试（瞬时 5xx/429/网络错退避重试 ≤2 次；4xx/本地 180s 超时不重试）；② 采购清单常备调料别名归一（`commonSeasoningsCanon`，跳过判定与 isHave 同口径）；③ schema 补两索引 `MealSession([familyId,status,targetTime])` + `Feedback([createdAt])`（db push 增量，prod 部署自动建）；④ `generateDishRecipeAction` 的 `recipe.create` catch P2002→友好提示（并发补谱不再抛原始错）；⑤ 主食兜底从写死「白米饭」退到「任意 isStaple 菜」+ 查无则 console.warn；⑥ `flexNum` 抽第一个数字 token（修 "2-3"/"2~3" 区间陷阱）；⑦ `DishImage` emoji 降级加 `role=img + aria-label={name}`（无障碍）；⑧ `finishCookingAction` 的 FamilyDish upsert 批量走 `$transaction`（消 N+1）。
- **仍刻意未做（需 schema 迁移设计，单独做）**：`Dish.canonicalKey` 去重字段（要归一表 + 改 3 条 upsert + 回填存量）、`MenuDish.dishId` 改可空+SetNull（涉及下游多处非空假设，侵入大）。另蒸锅占灶漏报 / 混单位 totalText 等 nit 级未动。详见会话评测报告。
